DROP MATERIALIZED VIEW IF EXISTS hourly_rentals;

CREATE MATERIALIZED VIEW IF NOT EXISTS hourly_rentals AS (
    WITH cross_table AS (SELECT
        d.date_time AS data_partenza,
    ...
    WHERE EXTRACT('hour' FROM d.date_time) BETWEEN 7 and 24
    ORDER BY nome, date_time ASC)
