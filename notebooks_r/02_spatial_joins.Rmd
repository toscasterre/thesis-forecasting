...
station_outflow = pd.read_csv("../data/bikemi_csv/station_daily_outflow.csv", parse_dates=[0], index_col=[0], dtype={"stazione_partenza": "string"})
...
geo_outflow_nil = geopandas.sjoin(geo_outflow, nil, how="inner", op = "intersects") \
    .rename(columns = {"geometry": "nil_geometry", "index_right" : "nil_number", "nil": "nil_name"}) \
    .set_geometry("nil_geometry") \
    .astype({"stazione_partenza": "string", "municipio": "string", "nil_number": "string", "nil_name": "string"})
...
```{python}
geo_outflow_nil.groupby("nil_name")["stazione_partenza"].count().sort_values(ascending=False).reset_index()
```
...
```{python}
nils_and_stations = pd.DataFrame(geo_outflow_nil.reset_index().groupby("nil_name")["stazione_partenza"].unique()).reset_index() \
    .assign(count = lambda x: x["stazione_partenza"].apply(lambda x: len(x))).sort_values("count")
...
```{python}
nil_daily_rentals = geo_outflow_nil.reset_index() \
    .groupby(["nil_name", "giorno_partenza"], as_index=False)["count"].sum() \
